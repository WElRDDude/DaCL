...
FileManager::FileManager(const std::string &bufferDir, const std::string &eventDir)
    ...
    std::error_code ec;
    if (!std::filesystem::exists(bufferDir_, ec) && !std::filesystem::create_directories(bufferDir_, ec)) {
        throw std::filesystem::filesystem_error("Cannot access or create buffer directory", 
                                               bufferDir_, ec);
    }
    if (!std::filesystem::exists(eventDir_, ec) && !std::filesystem::create_directories(eventDir_, ec)) {
        throw std::filesystem::filesystem_error("Cannot access or create event directory", 
                                               eventDir_, ec);
    }
}
...
void FileManager::copyEventSegments(const std::vector<std::string> &segments, 
    ...
    if (!std::filesystem::create_directories(eventDir_, ec) && ec) {
        throw std::runtime_error("Failed to create event directory: " + ec.message());
    }

    for (size_t i = 0; i < segments.size(); ++i) {
        const std::string dest = eventDir_ + "/" + timestamp + "_" + warningType + "_" + suffix + "_" + std::to_string(i) + ".mp4";
        
        // Copy file with error checking
        try {
            std::filesystem::copy(segments[i], dest, std::filesystem::copy_options::overwrite_existing);
        } catch (const std::filesystem::filesystem_error& e) {
            throw std::runtime_error("Failed to copy segment " + segments[i] + " to " + dest + ": " + e.what());
        }

        // Apply overlay using ffmpeg - improved command construction and error handling
        const std::string tempDest = dest + "_temp.mp4";
        const std::string cmd = "ffmpeg -y -i " + dest + " -vf \"movie=" + overlayFile + 
...
void FileManager::cleanOldSegments(int maxMinutes) {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(bufferDir_, ec)) {
        if (ec) {
        ...
        try {
            ...
            const auto age = std::chrono::duration_cast<std::chrono::minutes>(now - sctp);
