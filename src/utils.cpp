...
Config::Config(const std::string &filename) {
  ...
  try {
    ...
    throw std::runtime_error("Configuration value out of range: " +
                             std::string(e.what()));
  }
}
...
std::string currentTimestamp(const CANListener *canListener) {
  ...
  if (canListener != nullptr) {
    ...
    std::snprintf(buf, sizeof(buf), "%04d%02d%02d_%02d%02d%02d",
                  canListener->getYear(), canListener->getMonth(),
                  canListener->getDay(), canListener->getHour(),
                  canListener->getMinute(), canListener->getSecond());
  } else {
    ...
    if (pos != std::string::npos) {
      try {
        ...
        if (idStr.empty() || label.empty()) {
          std::cerr << "Warning: Skipping malformed CAN warning entry: " << item
                    << std::endl;
          continue;
        }
      ...
      std::cerr
          << "Warning: Skipping malformed CAN warning entry (missing comma): "
          << item << std::endl;
    }
  }
...
uint32_t extractSignal(const uint8_t *data, int startBit, int length,
                       bool isLittleEndian, double factor, double offset) {
  // Input validation
  if (data == nullptr) {
  ...
  if (endBit >= MAX_CAN_FRAME_SIZE * BITS_PER_BYTE) {
    throw std::invalid_argument(
        "Signal extends beyond standard CAN frame size");
  }
  ...
  for (int i = 0;
       i < (length + startBitInByte + BITS_PER_BYTE - 1) / BITS_PER_BYTE; ++i) {
    if (isLittleEndian) {
      // Little-endian: LSB first
      rawValue |= static_cast<uint64_t>(data[startByte + i])
                  << (i * BITS_PER_BYTE);
    } else {
