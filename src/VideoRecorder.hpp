...
/**
 * @class VideoRecorder
 * @brief Manages continuous video recording with circular buffering and event capture
 * 
 ...
 * 
 * @note Thread Safety: All public methods are thread-safe using internal mutexes.
 *       The run() method should be executed in a dedicated thread.
 */
...
public:
     ...
     */
    explicit VideoRecorder(const std::string &bufferDir, 
                          int segmentSeconds, 
                          int bufferMinutes, 
...
private:
    ...
    bool postTriggerActive_;           ///< Flag indicating post-trigger recording in progress
    int postTriggerSegmentsLeft_;      ///< Remaining segments for post-trigger recording
    ...
    static constexpr int MAX_BUFFER_FILES = 60;  ///< Maximum files in circular buffer
};