...
std::string OverlayRenderer::renderOverlay(int speed, 
    ...
    cv::putText(img, "Speed: " + std::to_string(speed) + " km/h", 
               cv::Point(TEXT_START_X, LINE_HEIGHT), 
               cv::FONT_HERSHEY_SIMPLEX, FONT_SIZE, COLOR_SPEED, TEXT_THICKNESS);
    ...
    cv::putText(img, "Warning: " + warningType, 
               cv::Point(TEXT_START_X, LINE_HEIGHT * 4), 
    ...
    try {
        ...
        throw std::runtime_error("OpenCV error while saving overlay: " + std::string(e.what()));
    }
